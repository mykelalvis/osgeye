...
public class MissingPackagesCommand extends AbstractExecuteOnBundlesCommand
{
  ...
  public MissingPackagesCommand(ServerState bundleStore)
  {
  ...
  {
    return "Displays unwired packages for bundles that are not wired.";
  }
  ...
  protected void executeOnBundles(List<Bundle> matchingBundles, List<String> subcommands) throws InvalidCommandException
  {
    ...
    for (Bundle bundle : matchingBundles)
    {
      ...
      {
        printMissingPackages(bundle, printMandatory, printOptional, allBundles);
      }
    }
  ...
  private void printMissingPackages(Bundle bundle, boolean printMandatory, boolean printOptional, List<Bundle> allBundles)
  {
    List<ImportedPackage> missingPackages = diagnosisUtils.findMissingImports(bundle.getManifest(), printMandatory, printOptional, allBundles);
